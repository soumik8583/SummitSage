 ...
 *
 * Sends mail straight from a Gmail account over SMTP (Nodemailer), so the
 * notification arrives reliably without depending on any third-party widget
 * or browser-side service. One branded template covers every form type
 ...
 *                       Defaults to soumikmondal723@gmail.com.
 *   GMAIL_APP_PASSWORD  A 16-character Google "App Password" for that account
 *                       (NOT the normal login password). Requires 2-Step
 *                       Verification: https://myaccount.google.com/apppasswords
 *   NOTIFY_EMAIL        Optional. Where submissions are delivered. Use a
 *                       comma-separated list for multiple recipients.
 *                       Defaults to the two Summit Sage inboxes.
 */

const nodemailer = require('nodemailer');

const GMAIL_USER = process.env.GMAIL_USER || 'soumikmondal723@gmail.com';
const GMAIL_APP_PASSWORD = (process.env.GMAIL_APP_PASSWORD || '').replace(/\s+/g, '');
// Recipients of every submission. Comma-separated so Gmail delivers a single
// message to both business inboxes. Override with the NOTIFY_EMAIL env var.
const NOTIFY_EMAIL =
  process.env.NOTIFY_EMAIL ||
  'soumikmondal723@gmail.com, rupak9609@gmail.com';
...
function getTransporter() {
  if (!isEmailConfigured()) {
    throw new Error(
      'Email is not configured: set the GMAIL_APP_PASSWORD environment variable.'
    );
  }
  if (!transporter) {
